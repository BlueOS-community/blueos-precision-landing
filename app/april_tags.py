...
def detect_april_tags(image: np.ndarray, tag_family: str, target_id: int, quad_decimate: int, include_augmented_image: bool) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        quad_decimate: Decimation factor for AprilTag detection (1=most precise/slowest, 4=fastest/least precise)
    ...
    try:
        # Create detector
        global detector, detector_quad_decimate_last
        if detector is None or detector_quad_decimate_last != quad_decimate:

            # record quad_decimate so we can detect changes later
            detector_quad_decimate_last = quad_decimate

            # delete existing detector if it exists
            ...
            logger.debug(f"{logging_prefix_str} created detector with quad_decimate={quad_decimate}")
