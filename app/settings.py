...
# get apriltag accuracy
def get_apriltag_accuracy():
    """
    Get the AprilTag quad_decimate setting

    Returns:
        int: The AprilTag quad_decimate value (1-4, default: 1)
    """
    settings = get_settings()
    return settings.get('apriltag', {}).get('accuracy', DEFAULT_SETTINGS['apriltag']['accuracy'])


# update AprilTag settings
def update_apriltag_settings(family, target_id, accuracy):
    """
    Update AprilTag settings
    ...
    Args:
        ...
        accuracy (int): The quad_decimate value (1-4)
    ...
    try:
        ...
        settings['apriltag']['accuracy'] = accuracy
...
# get image correction undistort enabled setting
def get_undistort_enabled():
    """
    Get the image undistortion enabled setting

    Returns:
        bool: True if undistortion is enabled, False otherwise (default: True)
    """
    settings = get_settings()
    return settings.get('image_correction', {}).get('undistort_enabled', DEFAULT_SETTINGS['image_correction']['undistort_enabled'])


# update image correction undistort enabled setting
def update_undistort_enabled(enabled):
    """
    Update image undistortion enabled setting

    Args:
        enabled (bool): Whether to enable image undistortion
    ...
    try:
        ...
        # Ensure image_correction section exists
        if 'image_correction' not in settings:
            settings['image_correction'] = {}

        settings['image_correction']['undistort_enabled'] = enabled

        save_settings(settings)
        return True
    except Exception as e:
        logger.error(f"Error updating undistort enabled setting: {e}")
        return False
